...
const child = spawn(
  process.execPath,
  [pipeline, ...platformWrapperArguments("instagram", forwardedArguments)],
  { stdio: "inherit" },
);

child.once("error", (error) => {
  console.error(`[ig-thumbs] unable to start shared pipeline: ${error.message}`);
  process.exitCode = 1;
});
child.once("exit", (code, signal) => {
  if (signal) {
    console.error(`[ig-thumbs] shared pipeline stopped by ${signal}`);
    process.exitCode = 1;
    return;
  }
